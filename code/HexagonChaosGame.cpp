	...
	while (window.isOpen())
	{
		...
		*/
		...
		if (chaosPoints.size() > 0)
		{
			for (int i = 0; i < 10; i++)
			{
				int randomVertex = rand() % 6;
				float midX = ((1 - 0.667) * chaosPoints.back().x) + 0.667 * baseVertices[randomVertex].x;
				float midY = ((1 - 0.667) * chaosPoints.back().y) + 0.667 * baseVertices[randomVertex].y;
		...
		*/
		...
		if (baseVertices.size() < 6 || chaosPoints.size() == 0)
		{
